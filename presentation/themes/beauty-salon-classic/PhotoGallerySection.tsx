          ...
          >
            {variant === 'grid' && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-6xl mx-auto">
                {images.map((image, i) => (
                  <div key={image.id || i} className={`relative aspect-square overflow-hidden group shadow-sm ${radiusClass}`}>
                    <Image
            ...
            {variant === 'masonry' && (
              <div className="columns-2 md:columns-3 lg:columns-4 gap-4 max-w-6xl mx-auto space-y-4">
                {images.map((image, i) => (
                  <div key={image.id || i} className={`relative overflow-hidden group shadow-sm break-inside-avoid ${radiusClass}`} style={{ aspectRatio: i % 3 === 0 ? '3/4' : i % 2 === 0 ? '4/3' : '1/1' }}>
                    <Image
                      src={image.filePath || `https://picsum.photos/seed/gallery-${i}/600/600`}
                      alt={image.altText || `Gallery image ${i}`}
                      fill
            ...
            {variant === 'carousel' && (
              <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-8 max-w-6xl mx-auto hide-scrollbar">
                {images.map((image, i) => (
                  <div key={image.id || i} className={`relative flex-none w-4/5 md:w-1/3 aspect-[4/5] overflow-hidden group shadow-sm snap-center ${radiusClass}`}>
                    <Image
                      src={image.filePath || `https://picsum.photos/seed/gallery-${i}/600/600`}
                      alt={image.altText || `Gallery image ${i}`}
                      fill
